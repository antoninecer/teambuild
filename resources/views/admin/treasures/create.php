...
<div class="card">
    <form action="/admin/games/<?= (int) $game['id'] ?>/treasures" method="POST">
        <div class="form-grid">
            ...
            <div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="max_claims" name="max_claims" value="<?= htmlspecialchars($old['max_claims'] ?? '', ENT_QUOTES, 'UTF-8') ?>" min="1" placeholder="Neomezeno">
                </div>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 20px; flex-wrap: wrap; margin: 20px 0;">
            ...
            <div class="checkbox-group">
                <input type="checkbox" id="is_enabled" name="is_enabled" value="1" <?= array_key_exists('is_enabled', $old ?? []) ? ($old['is_enabled'] ? 'checked' : '') : 'checked' ?>>
                <label for="is_enabled">Aktivní</label>
            </div>
        </div>

        <div style="margin-top: 30px; border-top: 1px solid var(--line); padding-top: 20px;">
            <button type="submit" class="btn btn-primary">Vytvořit poklad</button>
        </div>
    </form>
</div>
